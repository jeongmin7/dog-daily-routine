...
- 로드맵 남은 것: **프론트 리팩터**(이제 안전망 있음 — 큰 컴포넌트 `dog-monitoring`·`record-form`·`dog-medications`는 react-query 모킹이 필요해 테스트 미작성 상태) · **MVP 1b-ii**(약 알림 — cron+채널, 보류) · **MVP 4**(NestJS+Docker 분리).
...
- `tests/` — API 회귀 테스트 `*.test.ts`(node, `helpers.ts`) + 컴포넌트 테스트 `*.test.tsx`(jsdom, `setup-dom.ts`). 환경 분리는 `vitest.config.ts`(루트)의 `test.projects`
