...
- ⚠️ **globals.css의 컴포넌트 클래스는 `@layer` 밖이라 Tailwind 유틸리티를 *항상* 이긴다** (소스 순서 무관 — unlayered가 layered보다 우선). `.caption`에 `text-accent`를, `.row`에 `items-start`를 얹어도 안 먹는다. 색·정렬을 바꿔야 하면 그 컴포넌트 클래스를 쓰지 말고 유틸리티만으로 조립할 것. (근본 해결은 커스텀 클래스를 `@layer components`로 옮기는 것인데 앱 전체 시각에 영향이라 별도 작업으로 미뤄둠.)
